...
// Handles multi-service selection, live toast feedback, validation, and AJAX backend submission.
...
class ProjectBuilder {
  constructor() {
    ...
    this.currentStep = 1; // 1 = service selection, 2 = requirement form, 3 = success
    this.initElements();
    this.bindEvents();
  ...
  open(preselectedServiceName = null) {
    ...
    if (preselectedServiceName) {
      // Find card and select if not already
    ...
    try {
      ...
      if (response.ok && data.success) {
        if (this.successReference) {
          this.successReference.textContent = data.projectId || 'ELY-1004';
        }

        // WhatsApp direct link button hookup
        const whatsappBtn = document.getElementById('success-whatsapp-link');
        if (whatsappBtn) {
          const waMsg = encodeURIComponent(`Hi ELYRA Visual Studio! I just submitted project request ${data.projectId} for ${businessName}. Looking forward to discussing!`);
        ...
        const msg = data.errors ? data.errors.join(' ') : (data.message || 'Submission failed.');
        this.showToast(msg, 'warning');
      }
    } catch (err) {
      console.error('Submission error:', err);
      this.showToast('Network error while sending request. Please try again.', 'warning');
    } finally {
      if (this.submitBtn) {
